...
async function syncDataFromBackend() {
  try {
    const [txn, mats, hist] = await Promise.all([
    ...
    console.warn("Backend sync failed, using local demo data:", err.message);
  }
}
...
async function confirmBorrow() {
  ...
  const rollNo   = document.getElementById("borrow-roll")?.value.trim();
  const resource = document.getElementById("borrow-resource")?.value.trim();
  const type     = document.getElementById("borrow-type")?.value || "Other";
  const dueDate  = document.getElementById("borrow-due")?.value;
  if (!name || !rollNo || !resource || !dueDate) { alert("Please fill all required fields."); return; }
  try {
    await apiFetch("/api/transactions/borrow", {
      method: "POST",
      body: JSON.stringify({ student: name, rollNo, resource, type, dueDate }),
    });
...
async function confirmReturn() {
  ...
  try {
    ...
    showSuccessToast(`✅ Transaction ${txnId.toUpperCase()} marked as returned!`);
  } catch (err) {
    alert(err.message);
  }
}
...
async function upvoteMaterial(id) {
  try {
    const data = await apiFetch(`/api/materials/${id}/upvote`, { method: "POST" });
    matVotes[id] = data.votes;
...
async function handleUpload(e) {
  ...
  const subject = document.getElementById("upload-subject")?.value || "OTHER";
  const semesterText = document.getElementById("upload-semester")?.value || "Semester 1";
  const semester = parseInt((semesterText.match(/\d+/) || ["1"])[0], 10);
  const uploader = document.getElementById("upload-name")?.value.trim() || "Anonymous";
  const fileName = document.getElementById("upload-file")?.files?.[0]?.name || "material.pdf";
  const fileType = (fileName.split(".").pop() || "PDF").toUpperCase();

  if (!title) return;
  try {
    await apiFetch("/api/materials/upload", {
      method: "POST",
      body: JSON.stringify({ title, subject, semester, tags: tagList, uploader, fileType }),
    });
    await syncDataFromBackend();
    filterMaterials();
